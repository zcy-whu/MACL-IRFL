...
class Preprocessor:
    ...
    def preprocess_code_with_multiprocess(self, items):
        ...
        code, path, idx = items
        code = self.remove_import_and_package(code)  # 直接把import和import之前的注释全部去除了
        tokens, camel_word_split_record = self._preprocess(code)
        path_tokens, path_camel_word_split_record = self._preprocess(path)
        return tokens, camel_word_split_record, path_tokens, path_camel_word_split_record, idx
